...
void moveMinezone(double sideWidth, double sideHeight);
...
int main(int argc, char **argv){
  ...
  int corners = 2; //incase for later development that the zones are no longer rectangular
  double mineZone[corners]; //an array also just incase for later
...
while(errors == true){
  ...
  std::cin >> mineZone[1];
    ...
    break;
		...
		moveMinezone(mineZone[0],mineZone[1]);
...
void moveMinezone(double sideWidth,double sideHeight){
		...
		move_v3(0.3, sideHeight, true);
		rotate (0.3, degree2radian(90), true);
    move_v3(0.3, 0.5, true);
    rotate (0.3, degree2radian(90), true);
    move_v3(0.3, sideHeight, true);
		rotate (0.3, degree2radian(90), false);
    move_v3(0.3, 0.5, true);
    rotate (0.3, degree2radian(90), false);
    }
}
...
void move_v1(double speed, double distance, bool isForward){
	...
	try{
		...
		listener.lookupTransform("/base_link", "/map",ros::Time(0), init_transform);
	}
	...
	do{
		...
		try{
			...
			listener.waitForTransform("/base_link", "/map", ros::Time(0), ros::Duration(10.0) );
			//Once the transform is found,get the initial_transform transformation.
			listener.lookupTransform("/base_link", "/map",ros::Time(0), current_transform);
		}
		catch (tf::TransformException & ex){
			ROS_ERROR(" Problem %s",ex.what());
			ros::Duration(1.0).sleep();
...
double rotate(double angular_velocity, double radians,  bool clockwise)
{
	...
	TFListener.waitForTransform("base_link", "map", ros::Time(0), ros::Duration(1.0));


	//record the starting transform from the odometry to the base frame
	TFListener.lookupTransform("base_link", "map", ros::Time(0), init_transform);
	...
	while (!done )
	{
		...
		{
			TFListener.waitForTransform("base_link", "map", ros::Time(0), ros::Duration(1.0));
			TFListener.lookupTransform("base_link", "map", ros::Time(0), current_transform);
		}
		catch (tf::TransformException ex)
		{
			ROS_ERROR("%s",ex.what());
